Invoke @rbxspec-swe to establish the Senior Roblox Engineer persona for this session. Dispatch @rbxspec-swe for each feature spec worker.
When asked to /rbxspec.implement, execute an orchestrator loop that dispatches one subagent per feature spec.

## Prerequisite

- If no path is given, stop: "Usage: `/rbxspec.implement <task-path>`. Provide a task directory from `.rbxspec/tasks/`."
- Resolve: PROGRESS.md path → its directory, directory → check for PROGRESS.md, task name → `.rbxspec/tasks/<name>/PROGRESS.md`.
- If not found, stop: "Task directory not found. Run `/rbxspec.plan` first."

## Guardrails

1. Run the full loop continuously. Do not stop mid-run for checkpoints or TODO lists.
2. Never tell the user to rerun `/rbxspec.implement`. Continue dispatching while runnable work remains.
...
- Workers re-read `.rbxspec/CONTEXT.md` and `AGENTS.md`/`CLAUDE.md` fresh at W1
...
- Read `.rbxspec/CONTEXT.md` when present. Treat it as the primary source of truth for project context.
...
- Dispatch @rbxspec-swe for the selected feature spec. Include the Worker Protocol below as task context.
...
**partial:** The worker saved incremental progress. The spec remains `active` with updated state. Log the `phase_reached` and `note` from the result. The orchestrator re-dispatches this spec at S3 on the next loop iteration. Increment a retry counter for this spec. If retries exceed 3, the orchestrator may implement the spec directly — read @rbxspec-swe for the Worker Protocol and follow W1–W6 inline.
...
- Never ask to rerun `/rbxspec.implement`.
...
1. **Evidence-required validation.** Never mark a validate as passed unless you executed it and captured evidence in the same run. Evidence means verbatim output: test results, exit codes, file listings, command output, or Studio playtest observations. A validate with no evidence is treated as a failure — do not claim it passed.
...
2. Read `.rbxspec/CONTEXT.md` when present for project context and conventions.
3. Read AGENTS.md/CLAUDE.md if present.
4. Read PRD. Extract all AC-*/EC-* IDs.
5. Read assigned feature spec. Verify sections: Goal, Contracts, Files, Actions, Validates, Done.
6. Parse all fenced code blocks (config, allowlist, state, action, decision, validate).
7. If a section or block is missing, stop and report the first missing item.
8. Mark spec `active` in Registry. Update Active section with filename, phase `W1`, resume note, timestamp.
9. If another spec is already `active`, stop and report it.

Gate: all sections present, all blocks valid, spec marked active, no other active spec.

### W2 - Resolve Decisions

10. For each `decision` block, resolve using the process above.
11. Write the updated state block after each decision.
12. After all decisions are resolved, write the final state.

### W3 - Execute Actions

13. Topologically sort actions by `depends_on` (preserve document order for ties).
14. For each action in order:
...
15. Update Active (phase `W3`, current step, timestamp) after each major action.

Gate: all actions executed or skipped, no abort-level failures.

### W4 - Audit Implemented Work

16. Diff review — compare actual file changes against the spec's Files table and Contracts section:
    ...
    - every Data contract (DataStore schema fields, session state shape, Attributes) is present in code
...
18. If all files and contracts match, proceed to W5.

Gate: all planned files and contracts match spec. No mismatches remain.

### W5 - Run Validates

19. For each `validate` block (grouped by type: base → edges → e2e):
    ...
    c. Execute the validate:
       - `run_command`: run the command and check its result against `expect`.
       - `studio_playtest`: connect to the Roblox Studio MCP server, execute the planned playtest steps (read game tree, start playtest, observe output logs, inspect instances, capture screenshots), and record observations against `expect`. If Studio MCP is unavailable or disconnected, record `failed` evidence stating the dependency is missing — never claim the playtest passed.
    d. If result matches `expect`: pass. Record evidence in `state.evidence` — map the validate id to a brief summary of the passing result (e.g., "all 12 tests pass, exit 0" or "playtest observed: coins replicated to leaderstats"). Write the updated state block to the feature spec file.
...
20. Never claim a validate passed unless you ran it and captured evidence in the same run.
21. Never claim a validate passed without recording evidence in `state.evidence`. A validate with no evidence entry is treated as a failure.
...
22. Done-to-validate mapping — for each Done checkbox in the spec, identify which validate block ids prove it. A Done item can only be marked `[x]` if at least one mapped validate id has a passing result recorded in `state.evidence`. If no mapped validate has evidence, the Done item remains `[ ]`.
23. Write an attestation in the PROGRESS.md Active section listing each Done item, its mapped validate ids, and the evidence summary for each validate. This attestation must be present before the spec can be marked done.
24. Check every Done checkbox with evidence. Mark `[x]` only with proof from `state.evidence`.
25. If review finds issues, batch-fix, rerun affected validates, repeat that review pass.
26. Mark spec `done` in Registry.
27. Update Active to: Spec `None`, Phase `idle`, note about next spec.
28. Update PRD `## Features`: this feature's status → [IMPLEMENTED].
29. Update state block: `status: done`, `finished_at: <ISO timestamp>`, `current_action: null`.
30. Return compact result: status, spec id/title, files changed, verification summary, coverage addressed.

## Constraints

- Orchestrator coordinates subagents; it does not implement feature specs directly for the first 3 dispatch attempts. If a subagent fails, returns empty, or returns partial, the orchestrator must re-dispatch. After 3 failed dispatches for the same spec, the orchestrator may fall back to direct implementation — but must read @rbxspec-swe for the Worker Protocol and follow W1–W6 inline, never skip validates or checkpointing. Never fall back to writing code, running tests, or creating files itself before exhausting 3 retries.
...
- Keep logic server-authoritative; never move gameplay decisions to the client to make a test pass
...
- Never tell the user to rerun `/rbxspec.implement`
