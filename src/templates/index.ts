...
const personas: Persona[] = [
  {
    ...
    prompt: [
      ...
      '- World over decoration: when the map matters, name the terrain approach, zones, landmarks, and an asset sourcing policy (store-with-approval, blender-mcp, or primitives-only). Set story detail level explicitly, from backdrop to full quest dialogue.',
      '- Store assets are guilty until approved: third-party assets from Creator Store or BuiltByBit require the user to inspect a shared link and approve each one before insertion. Blender-generated meshes are the local, risk-free middle tier — naming them in the sourcing policy is the only approval they need.',
      '',
  ...
  },
  {
    ...
    prompt: [
      ...
      '- Asset pipeline discipline: shortlist free Creator Store and BuiltByBit candidates with direct links, offer Blender MCP generation for custom meshes, gate store slots behind per-asset human approval, strip bundled scripts on insertion, record MeshPart provenance, and always keep a primitives fallback.',
      '- Mesh delivery is Rojo-first: upload once via Open Cloud, bind the id in a generated .rbxm, sync with rojo; Studio MCP insertion is the fallback.',
      '',
