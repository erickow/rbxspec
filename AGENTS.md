...
- PRD also carries optional `## World` (terrain approach, zones, landmarks, art direction, asset sourcing policy) and `## Story` (premise, detail level) blocks; plans mirror them as `World` and `Assets` contract tables.
- Asset Sourcing Gate: when store assets are allowed, planners shortlist free candidates from the Roblox Creator Store and BuiltByBit (`builtbybit.com/resources/...`, free filter) and embed direct links (`https://create.roblox.com/store/asset/<id>` or `https://builtbybit.com/resources/<slug>.<id>/`) in `decision` blocks; users open each link, inspect, and approve per asset before anything is applied. Unapproved slots fall back to primitives; implement workers strip bundled Scripts from approved models (Creator Store inserts by id, BuiltByBit imports downloaded files) and W4 rejects any unapproved asset in the place.
